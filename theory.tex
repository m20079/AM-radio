...
ここで、\(\omega_0\)は共振周波数、\(\omega_1\)と\(\omega_2\)は共振周波数時の振幅を\(\sqrt{2}\)で割った値の周波数である。
...
この同調回路の共振周波数、Q値を考えるために\wfig{kyo}のような回路を考える。
...
この回路のQ値は\(\infty\)である。
また、\wfig{kyo2}のようにコイルとコンデンサの抵抗成分を考慮した場合の並列回路を考えてみる。
...
レーダー方程式（反射とかは無視、アンテナゲインも無視、簡易的なやつ）を\weq{radar}に示す。
