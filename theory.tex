...
また，コイルとコンデンサの抵抗成分を考慮した場合の並列回路のアドミタンスは，\weq{impedance2}のようになる。
...
この時の周波数\(f\rm{[Hz]}\)は\weq{kyousin2}\(\sim\)\weq{kyousin}のようにして決定することが出来る。
...
\subsubsection{Q値}
Q値とは，同調回路において，共振周波数以外で信号の振幅がどれほど小さいかを表す値である。これが高い程，性能のよい同調回路であると言える。Q値は，\weq{Q}で与えられる。

\begin{align}
	Q値 =\frac{A_0}{\omega_2 - \omega_1}
\end{align}
