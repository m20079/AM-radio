...
\wfig{indad}、\wfig{indad5}、\wfig{insda5}にアンテナ一号機、二号機、三号機を用いて4段階増幅（\(5.7^4\)倍）したときの波形を示す。
...
\wtab{ant2}に示されるように、全ての局を聞くことができなかった。我々は，これはバリアブルコンデンサが機能していない為だと考えた。
...
\wfig{inda4}\wfig{inda5}に、\wfig{inda3}の測定結果を示す。
また\wtab{Q}に、各条件での共振周波数とQ値を示す。
...
\wtab{ant3}に、\wfig{inda4}の結果から、バリアブルコンデンサの最小と最大を計算した（一号機はグラフの形が変なので、二号機のグラフを用いた）。
...
\weq{radar}のレーダー方程式によって求めた電力密度と、教室から送信局までの方角を\wtab{aaa}に示す。
...
文化放送を基準に、TBSラジオがどのぐらい混信しているかを\weq{k_3}\(\sim\)\weq{k_2}に示す。
